...
// ── POST /api/registration/transfer ─────────────────────────
export async function POST(req: NextRequest) {
  try {
    ...
    const [maxRows] = await pool.query<RowDataPacket[]>(
      "SELECT COALESCE(MAX(sr_no), 0) + 1 AS next_sr FROM transfer_registrations"
    );
    ...
    const [result] = await pool.query<ResultSetHeader>(
      `INSERT INTO transfer_registrations
         (sr_no, name, father_name, cnic, phone, maker, model_year, color, engine_cc,
          chassis_no, engine_no, reg_no_new, reg_no_old,
      ...
      [
        ...
        b.maker, b.modelYear, b.color || null, b.engineCC || null,
        b.chassisNo, b.engineNo, b.regNoNew || null, b.regNoOld || null,
    ...
    console.error("[POST /api/registration/transfer]", err);
