...
const INIT_FORM: FormState = {
  ...
  maker: "", modelYear: "", regNoOld: "", regNoNew: "",
  chassisNo: "", engineNo: "",
  amount: "", remarks: "", date: "",
        ...
        <div style={{ position: "absolute", top: "calc(100% + 6px)", left: 0, background: "#fff", border: `1px solid ${BORDER}`, borderRadius: 12, padding: 16, zIndex: 500, boxShadow: "0 8px 32px rgba(0,0,0,.15)", minWidth: 290 }}>
          ...
          <div style={{ marginTop: 12, display: "flex", justifyContent: "flex-end" }}>
            <button onClick={() => { const t = today; setViewYear(t.getFullYear()); setViewMonth(t.getMonth()); selectDay(t.getDate()); }}
              style={{ background: "none", border: `1px solid ${BORDER}`, borderRadius: 6, padding: "5px 14px", fontSize: ".75rem", fontWeight: 600, color: TEAL, cursor: "pointer" }}>Today</button>
          </div>
        </div>
      )}
    </div>
...
function printReceipt(form: FormState, serialNumber: string, savedTime: string) {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>Transfer Sheet â€“ New Bilal Motors</title><style>${PRINT_CSS}</style></head><body><div class="page">
...
<div class="section"><div class="section-title">Vehicle Details</div><div class="row"><div class="f"><div class="f-label">Maker</div><div class="f-value">${val(form.maker)}</div></div><div class="f"><div class="f-label">Model Year</div><div class="f-value">${val(form.modelYear)}</div></div><div class="f"><div class="f-label">Registration No. (Old)</div><div class="f-value">${val(form.regNoOld)}</div></div><div class="f"><div class="f-label">Registration No. (New)</div><div class="f-value">${val(form.regNoNew)}</div></div></div><div class="row"><div class="f"><div class="f-label">Chassis No.</div><div class="f-value">${val(form.chassisNo)}</div></div><div class="f"><div class="f-label">Engine No.</div><div class="f-value">${val(form.engineNo)}</div></div></div></div>
<hr class="sec-divider"/>
    ...
    try {
      const res = await fetch("/api/registration/transfer", {
        ...
        body: JSON.stringify({
          ...
          regNoOld: form.regNoOld, regNoNew: form.regNoNew,
      ...
      <div style={s.tabsBar}>
        {TABS.map(tab => (
          <button key={tab} onClick={() => handleTabClick(tab)}
            style={{ ...s.tab, ...(tab === "Transfer" ? s.tabActive : {}) }}>
            {tab}
          </button>
        ))}
      </div>
      ...
      <div style={s.card}>
        ...
        {savedRecord && (
          <div style={s.savedBanner}>
            <div>
              <div style={s.savedBannerText}>âœ“ Transfer saved â€” SR-{savedSerial}</div>
        ...
        <div style={{ ...s.grid4, marginTop: 18 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 3fr", columnGap: 20, rowGap: 18 }}>
          <Field label="Amount">
            <input type="number" placeholder="Enter amount (PKR)" value={form.amount}
        ...
        <div style={s.footer}>
          {savedRecord ? (
            ...
            <>
              ...
              <button style={s.btnSave} onClick={saveTransfer} disabled={loading}>
                {loading ? "Savingâ€¦" : "Save Transfer"}
              </button>
            </>
          )}
        </div>
