...
const INIT_FORM: FormState = {
  ...
  chassisNo: "", engineNo: "", regNoNew: "", regNoOld: "",
  amount: "", remarks: "", date: "",
  fingerprint: "",
...
function DatePicker({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div ref={ref} className="tr-datepicker">
      <div onClick={() => setOpen(o => !o)} className="tr-input tr-date-trigger">
      ...
      {open && (
        <div className="tr-date-panel">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            <button onClick={prevMonth} className="tr-date-nav-btn">&#8249;</button>
            <span style={{ fontWeight: 700, fontSize: ".88rem", color: "#2a2d35" }}>{MONTH_NAMES[viewMonth]} {viewYear}</span>
            <button onClick={nextMonth} className="tr-date-nav-btn">&#8250;</button>
          </div>
          ...
          <div style={{ marginTop: 12, display: "flex", justifyContent: "flex-end" }}>
            <button onClick={() => { const t = today; setViewYear(t.getFullYear()); setViewMonth(t.getMonth()); selectDay(t.getDate()); }}
              className="tr-date-today-btn">Today</button>
          </div>
        </div>
      )}
    </div>
...
function SectionHead({ title, sub }: { title: string; sub: string }) {
  return (
    <div style={{ marginBottom: 20 }}>
      <p className="tr-section-title">{title}</p>
      <p className="tr-section-sub">{sub}</p>
    </div>
  );
}
...
function Field({ label, required = false, children, style }: FieldProps) {
  return (
    <div className="tr-field" style={style}>
      <label className="tr-field-label">{label}{required && <span className="tr-req">*</span>}</label>
      {children}
    </div>
  );
}
...
function TextInput({ value, onChange, placeholder, readOnly = false }: TextInputProps) {
  return (
    <input type="text" placeholder={placeholder} readOnly={readOnly} value={value}
      ...
      className={`tr-input ${readOnly ? "tr-input-readonly" : ""}`} />
  );
}

function SelectInput({ value, onChange, options, placeholder }: SelectInputProps) {
  return (
    <select value={value} onChange={(e) => onChange(e.target.value)} className="tr-input tr-select">
...
function SelectWithOther({ value: rawValue, onChange, options, placeholder }: SelectInputProps) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      ...
      {showCustom && (
        <input type="text" placeholder="Type custom value..." value={value} onChange={(e) => onChange(e.target.value)} className="tr-input" autoFocus />
      )}
    </div>
  );
}
...
function printReceipt(form: FormState, serialNumber: string, savedTime: string) {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>Transfer Sheet -- New Bilal Motors</title><style>${PRINT_CSS}</style></head><body><div class="page">
<div class="header"><div class="header-left"><h1>New Bilal Motors</h1><div class="shop-addr">Laiq Ali Chowk, Wah Cantt</div><div class="tagline">A trusted institution for buying and selling new and used motorcycles.</div></div><div class="header-right"><div class="sheet-title">Registration Sheet (Transfer)</div><table><tr><td style="font-size:10.5px;padding:1px 8px;color:#666;white-space:nowrap;">Serial No.:</td><td style="font-size:10.5px;font-weight:700;white-space:nowrap;">SR-${serialNumber}</td></tr><tr><td style="font-size:10.5px;padding:1px 8px 1px 0;color:#666;white-space:nowrap;">Date:</td><td style="font-size:10.5px;font-weight:700;white-space:nowrap;">${formatDate(form.date)}</td></tr><tr><td style="font-size:10.5px;padding:1px 8px 1px 0;color:#666;white-space:nowrap;">Time:</td><td style="font-size:10.5px;font-weight:700;white-space:nowrap;">${savedTime}</td></tr></table></div></div>
...
<div class="section"><div class="section-title">Vehicle Details</div><div class="row"><div class="f"><div class="f-label">Brand/Maker</div><div class="f-value">${val(form.maker)}</div></div><div class="f"><div class="f-label">Model Year</div><div class="f-value">${val(form.modelYear)}</div></div><div class="f"><div class="f-label">Color</div><div class="f-value">${val(form.color)}</div></div><div class="f"><div class="f-label">Engine CC</div><div class="f-value">${val(form.engineCC)}</div></div></div><div class="row"><div class="f"><div class="f-label">Chassis No.</div><div class="f-value">${val(form.chassisNo)}</div></div><div class="f"><div class="f-label">Engine No.</div><div class="f-value">${val(form.engineNo)}</div></div><div class="f"><div class="f-label">Registration No. (Old)</div><div class="f-value">${val(form.regNoOld)}</div></div><div class="f"><div class="f-label">Registration No. (New)</div><div class="f-value">${val(form.regNoNew)}</div></div></div></div>
<hr class="sec-divider"/>
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-start", gap: 20 }}>
      <div style={{ display: "flex", flexDirection: "column", gap: 8, minWidth: 210 }}>
        <button type="button" onClick={connectScanner} disabled={busy} className="tr-fp-btn"
          style={{
        ...
        <button type="button" onClick={captureFingerprint} disabled={!connected || busy} className="tr-fp-btn"
          style={{
        ...
        {value && (
          <button type="button" onClick={() => onChange("")} className="tr-fp-clear-btn">
            Clear
          </button>
        )}
...
export default function TransferRegistration() {
  ...
  const saveTransfer = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/registration/transfer", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...
          color: form.color, engineCC: form.engineCC,
          chassisNo: form.chassisNo, engineNo: form.engineNo,
          regNoNew: form.regNoNew, regNoOld: form.regNoOld,
          date: form.date, amount: form.amount, remarks: form.remarks,
  ...
  return (
    <div className={dmSans.className + " tr-shell"}>
      <style>{`
        .tr-shell { display: flex; align-items: flex-start; min-height: 100vh; background: #fafafb; }
        ...
        .tr-breadcrumb { font-size: .95rem; color: #16171b; font-weight: 700; }
        .tr-avatar {
        ...
        .tr-avatar:hover { background: #eef0f2; }

        .tr-tabs { display: flex; gap: 4px; padding: 0 32px; background: #fff; border-bottom: 1px solid ${BORDER}; flex-wrap: wrap; }
        .tr-tab {
          padding: 13px 6px; margin-right: 26px; font-size: .85rem; font-weight: 500; color: ${MUTED};
          background: none; border: none; border-bottom: 2px solid transparent; cursor: pointer;
          font-family: inherit; transition: color .15s, border-color .15s;
        }
        .tr-tab:hover { color: #2a2d35; }
        .tr-tab-active { color: #16171b; font-weight: 700; border-bottom: 2px solid ${TEAL}; }

        .tr-content { padding: 28px 32px 48px; }
        .tr-card {
          background: #fff; border: 1px solid ${BORDER}; border-radius: 14px;
          padding: 30px 34px 34px; box-shadow: 0 4px 18px rgba(20,20,25,.05);
        }

        .tr-section-title { font-size: .95rem; font-weight: 700; color: #16171b; margin-bottom: 3px; }
        .tr-section-sub { font-size: .78rem; color: ${MUTED}; }

        .tr-divider { border: none; border-top: 1px solid ${BORDER}; margin: 28px 0; }

        .tr-grid4 { display: grid; grid-template-columns: repeat(4, 1fr); column-gap: 20px; row-gap: 18px; }
        .tr-pay-row { display: grid; grid-template-columns: 1fr 3.12fr; column-gap: 20px; row-gap: 18px; }

        .tr-field { display: flex; flex-direction: column; gap: 6px; }
        .tr-field-label { font-size: .78rem; font-weight: 600; color: ${LABEL}; display: flex; align-items: center; gap: 4px; }
        .tr-req { color: ${ACCENT}; }
        .tr-hint { font-weight: 400; color: ${MUTED}; font-size: .72rem; margin-left: auto; }
        ...
        .tr-input:focus { border-color: ${TEAL}; box-shadow: 0 0 0 3px rgba(26,74,74,.1); background: #fff; }
        .tr-input-error { background: #fff8f7; border-color: #e74c3c; }
        .tr-input-readonly { background: #f0f2f5; color: ${MUTED}; cursor: not-allowed; }
        .tr-error-msg { font-size: .72rem; color: #e74c3c; font-weight: 600; margin-top: 1px; }
        ...
        .tr-datepicker { position: relative; }
        .tr-date-trigger { display: flex; align-items: center; justify-content: space-between; cursor: pointer; user-select: none; }
        .tr-date-panel {
        ...
        .tr-docs-row { display: flex; flex-wrap: wrap; gap: 10px 36px; }
        .tr-check-label { display: flex; align-items: center; gap: 9px; font-size: .82rem; color: ${LABEL}; cursor: pointer; user-select: none; }
        .tr-checkbox {
        ...
        .tr-checkbox:checked { background: ${TEAL}; border-color: ${TEAL}; }
        ...
        .tr-footer { display: flex; justify-content: flex-end; gap: 10px; margin-top: 30px; }
        .tr-btn {
        ...
        .tr-btn-save { background: ${TEAL}; color: #fff; }
        .tr-btn-save:hover { background: #143a3a; box-shadow: 0 6px 16px rgba(26,74,74,.25); }
        .tr-btn-save:disabled { opacity: .7; cursor: not-allowed; transform: none; }
        .tr-btn-reset { background: #fff; color: ${LABEL}; border: 1px solid ${BORDER}; }
        .tr-btn-reset:hover { background: #f4f5f7; }
        .tr-btn-print {
        ...
        .tr-btn-print:hover { background: #f0f6f6; }
        .tr-btn-new { background: ${TEAL}; color: #fff; border: none; padding: 9px 26px; border-radius: 8px; font-size: .85rem; font-weight: 600; cursor: pointer; font-family: inherit; transition: background .15s; }
        .tr-btn-new:hover { background: #143a3a; }
        ...
        .tr-saved-banner-text { font-size: .87rem; color: #1a7a4a; font-weight: 600; }

        .tr-toast {
          position: fixed; top: 28px; left: 50%; transform: translateX(-50%); padding: 12px 22px;
          border-radius: 8px; color: #fff; font-size: .85rem; font-weight: 600; z-index: 9999;
          box-shadow: 0 4px 16px rgba(0,0,0,.18); white-space: nowrap;
        }
        .tr-toast-success { background: #1a7a4a; }
        .tr-toast-error { background: #c0392b; }

        @media (max-width: 1100px) {
          .tr-grid4 { grid-template-columns: repeat(2, 1fr); }
          .tr-pay-row { grid-template-columns: 1fr; }
        }
        @media (max-width: 900px) {
          .tr-shell { flex-direction: column; }
        }
        @media (max-width: 640px) {
          .tr-content { padding: 20px 16px 32px; }
          .tr-card { padding: 22px 18px 26px; }
          .tr-grid4 { grid-template-columns: 1fr; }
          .tr-footer { flex-direction: column-reverse; }
          .tr-btn { width: 100%; }
        }
      `}</style>
      ...
      <div className="tr-main">
        <div className="tr-topbar">
          <span className="tr-breadcrumb">Registration Management</span>
          <button className="tr-avatar" aria-label="Account">
            <UserIcon size={18} />
          </button>
        </div>
        ...
        <div className="tr-tabs">
          {TABS.map(tab => (
            <button key={tab} onClick={() => handleTabClick(tab)}
              className={`tr-tab ${tab === "Transfer" ? "tr-tab-active" : ""}`}>
              {tab}
            </button>
          ))}
        </div>
          ...
          >
            ...
            {savedRecord && (
              <div className="tr-saved-banner">
                <div>
                  <div className="tr-saved-banner-text">✓ Transfer saved — SR-{savedSerial}</div>
                  <div style={{ fontSize: ".78rem", color: "#2a7a4a", marginTop: 2 }}>Saved at {savedTime}</div>
                </div>
                <div style={{ display: "flex", gap: 10 }}>
                  <button className="tr-btn-print" onClick={() => printReceipt(savedRecord, savedSerial, savedTime)}>
                  ...
                  <button className="tr-btn-new" onClick={resetForm}>+ New Entry</button>
                </div>
              </div>
            )}
            ...
            <div className="tr-grid4">
              ...
              <div className="tr-field">
                <label className="tr-field-label">CNIC<span className="tr-hint">(xxxxx-xxxxxxx-x)</span></label>
                <input type="text" placeholder="Enter Cnic" value={form.cnic} maxLength={15}
                  ...
                  className={`tr-input ${cnicError ? "tr-input-error" : ""}`} />
                {cnicError && <span className="tr-error-msg">Incorrect CNIC</span>}
              </div>
              <div className="tr-field">
                <label className="tr-field-label">Phone No.<span className="tr-hint">(03xx-xxxxxxx)</span></label>
                <input type="text" placeholder="Enter Phone Number" value={form.phone} maxLength={12}
                  ...
                  className={`tr-input ${phoneError ? "tr-input-error" : ""}`} />
                {phoneError && <span className="tr-error-msg">Incorrect Number</span>}
              </div>
            </div>
            ...
            <div className="tr-grid4">
              ...
              <Field label="Registration No. (Old)">
                <TextInput value={form.regNoOld} onChange={v => set("regNoOld", v)} placeholder="Old registration no." />
              </Field>
            ...
            <div className="tr-docs-row">
              {DOCS.map(doc => (
                <label key={doc} className="tr-check-label">
            ...
            <div className="tr-footer">
              {savedRecord ? (
                ...
                <>
                  <button className="tr-btn tr-btn-reset" onClick={requestReset}>Reset</button>
                  <button className="tr-btn tr-btn-save" onClick={saveTransfer} disabled={loading}>
